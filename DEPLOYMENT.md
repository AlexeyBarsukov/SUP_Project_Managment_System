...
2. Запустите `npm run test-simple` для простого теста подключения
3. Проверьте, что скопировали полный URL из Render Dashboard
4. Убедитесь, что URL начинается с `postgres://`
5. Проверьте, что в URL нет лишних пробелов или символов

**Если получаете ошибку "searchParams":**
- Это может быть связано с неправильным форматом URL
- Попробуйте пересоздать базу данных на Render
- Убедитесь, что в пароле нет специальных символов
- Проверьте, что URL скопирован полностью без обрезания
```
